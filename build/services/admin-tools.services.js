...
class AdminToolsService {
    ...
    async getTools(filters) {
        const { page, limit, search, category, status, badge, access_level, is_featured, sort_by, sort_order, } = filters;
        ...
        let baseQuery = (0, index_schema_1.default)(index_schema_1.T.TOOLS);
        if (search) {
        ...
        const tools = await baseQuery
            .clone()
            .select("id", "title", "slug", "category_slug", "tool_type", "short_description", "tags", "badge", "access_level", "tool_url", "status", "is_featured", "sort_order", "created_at", "updated_at")
    ...
    async getToolById(id) {
        const tool = await (0, index_schema_1.default)(index_schema_1.T.TOOLS)
            ...
            .leftJoin("tools_category_pages", "tools.category_slug", "tools_category_pages.category_slug")
            .select(
        // Tool
        "tools.id", "tools.title", "tools.slug", "tools.category_slug", "tools.tool_type", "tools.tags", "tools.short_description", "tools.badge", "tools.access_level", "tools.tool_url", "tools.status as tool_status", "tools.is_featured", "tools.sort_order", "tools.created_at", "tools.updated_at", 
        // Tool page
        "tool_pages.id as page_id", "tool_pages.page_title", "tool_pages.page_intro", "tool_pages.long_content", "tool_pages.features", "tool_pages.faqs", "tool_pages.meta_title as page_meta_title", "tool_pages.meta_description as page_meta_description", "tool_pages.meta_keywords as page_meta_keywords", "tool_pages.canonical_url as page_canonical_url", "tool_pages.noindex as page_noindex", "tool_pages.schema_markup", "tool_pages.status as page_status", 
        // Category - FIX: aliased category_slug to avoid ambiguity
        "tools_category_pages.category_slug as cat_slug", "tools_category_pages.page_title as category_page_title", "tools_category_pages.page_description as category_page_description", "tools_category_pages.page_intro as category_page_intro", "tools_category_pages.meta_title as category_meta_title", "tools_category_pages.meta_description as category_meta_description", "tools_category_pages.meta_keywords as category_meta_keywords", "tools_category_pages.canonical_url as category_canonical_url", "tools_category_pages.noindex as category_noindex", "tools_category_pages.status as category_status")
            .where("tools.id", id)
            .first();
        if (!tool)
            return null;
    ...
    async createTool(data) {
        ...
        try {
            ...
            const [createdTool] = await trx(index_schema_1.T.TOOLS)
                .insert({
                title: tool.title,
                slug: tool.slug,
                category_slug: tool.category_slug,
    ...
    async updateTool(id, data) {
        ...
        try {
            ...
            if (tool) {
                ...
                await trx(index_schema_1.T.TOOLS).where("id", id).update(updateData);
            }
            if (page) {
                // FIX: use explicit column alias to avoid ambiguity with category_slug
                const currentTool = await trx(index_schema_1.T.TOOLS)
                    .select("slug as tool_slug")
                    .where("id", id)
                    .first();
                if (!currentTool) {
                    throw new HttpException_1.default(404, "Tool not found during page update");
                }
                ...
                if (page.noindex !== undefined)
                    pageUpdateData.noindex = page.noindex;
                ...
                if (page.status !== undefined)
                    pageUpdateData.status = page.status;
                // If slug changed, update FK in tool_pages
                if ((tool === null || tool === void 0 ? void 0 : tool.slug) && tool.slug !== currentTool.tool_slug) {
     ...
     */
    async softDeleteTool(id) {
        const trx = await index_schema_1.default.transaction();
        try {
            const tool = await trx(index_schema_1.T.TOOLS).select("slug").where("id", id).first();
            if (!tool) {
                throw new HttpException_1.default(404, "Tool not found");
            }
            // Soft delete using status='archived'
            await trx(index_schema_1.T.TOOLS).where("id", id).update({
                status: "archived",
    ...
    async hardDeleteTool(id) {
        ...
        try {
            ...
            await trx(index_schema_1.T.TOOLS).where("id", id).delete();
            await trx.commit();
            return true;
        }
        catch (error) {
    ...
    async bulkUpdate(ids, updates) {
        const updateData = Object.assign(Object.assign({}, updates), { updated_at: index_schema_1.default.fn.now() });
        const updated = await (0, index_schema_1.default)(index_schema_1.T.TOOLS).whereIn("id", ids).update(updateData);
        return updated;
    }
    /**
     * Bulk delete tools.
    ...
    async bulkDelete(ids, permanent = false) {
        if (permanent) {
            const trx = await index_schema_1.default.transaction();
            try {
                const tools = await trx(index_schema_1.T.TOOLS).select("slug").whereIn("id", ids);
                const slugs = tools.map((t) => t.slug);
                await trx("tool_pages").whereIn("tool_slug", slugs).delete();
                const deleted = await trx(index_schema_1.T.TOOLS).whereIn("id", ids).delete();
                await trx.commit();
                return deleted;
            }
            catch (error) {
            ...
            try {
                ...
                const updated = await trx(index_schema_1.T.TOOLS).whereIn("id", ids).update({
                    status: "archived",
                    updated_at: index_schema_1.default.fn.now(),
                });
                await trx.commit();
     ...
     */
    async getCategories() {
        return (0, index_schema_1.default)("tools_category_pages")
            .leftJoin("tools", "tools_category_pages.category_slug", "tools.category_slug")
            .select("tools_category_pages.category_slug", "tools_category_pages.page_title", "tools_category_pages.page_description", "tools_category_pages.status", "tools_category_pages.created_at", "tools_category_pages.updated_at")
            .count("tools.id as tool_count")
            .groupBy("tools_category_pages.category_slug", "tools_category_pages.page_title", "tools_category_pages.page_description", "tools_category_pages.status", "tools_category_pages.created_at", "tools_category_pages.updated_at")
    ...
    async upsertCategory(categoryData) {
        const existing = await (0, index_schema_1.default)("tools_category_pages")
            .where("category_slug", categoryData.category_slug)
            .first();
        if (existing) {
            await (0, index_schema_1.default)("tools_category_pages")
                .where("category_slug", categoryData.category_slug)
                .update({
            ...
            await (0, index_schema_1.default)("tools_category_pages").insert({
     ...
     */
    async upsertCategoryTransaction(categoryData, trx) {
    ...
    async deleteCategory(slug) {
        const updated = await (0, index_schema_1.default)("tools_category_pages")
            .where("category_slug", slug)
            .update({
            status: "archived",
            updated_at: index_schema_1.default.fn.now(),
     ...
     */
    async getCategoryToolCount(slug) {
        const [{ count }] = await (0, index_schema_1.default)(index_schema_1.T.TOOLS)
            .where("category_slug", slug)
            .count("* as count");
    ...
    async getToolAnalytics(id) {
        ...
        if (!tool)
            return null;
        return Object.assign(Object.assign({}, tool), { analytics: {
                total_views: 0,
                unique_users: 0,
     ...
     * Duplicate tool
     */
    async duplicateTool(id, newSlug, newTitle) {
        const trx = await index_schema_1.default.transaction();
        try {
            const originalTool = await trx(index_schema_1.T.TOOLS).where("id", id).first();
            if (!originalTool) {
                throw new HttpException_1.default(404, "Tool not found");
            }
            const originalPage = await trx("tool_pages")
                ...
                .first();
            const [duplicatedTool] = await trx(index_schema_1.T.TOOLS)
                .insert({
    ...
    async restoreTool(id) {
        const trx = await index_schema_1.default.transaction();
        try {
            const tool = await trx(index_schema_1.T.TOOLS)
                .select("slug")
                .where("id", id)
                .where("status", "archived")
                .first();
            if (!tool) {
                throw new HttpException_1.default(404, "Tool not found or is not in archived state");
            }
            await trx(index_schema_1.T.TOOLS).where("id", id).update({
                status: "draft",
                updated_at: index_schema_1.default.fn.now(),
...
exports.default = AdminToolsService;
//# sourceMappingURL=admin-tools.services.js.map