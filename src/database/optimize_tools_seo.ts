...
// Define a structured interface for the AI output
interface AiSeoPayload {
  primary_focus_keyword: string;
  meta_title: string;
  meta_description: string;
  page_title: string;
  page_intro: string;
  long_content: string; // Detailed HTML guide
...
async function conductKeywordResearch(toolTitle: string): Promise<string[]> {
...
async function generateToolSeo(
  tool: any,
  suggestedKeywords: string[],
  engine: AIEngine,
  specifiedModel?: string | null
): Promise<AiSeoPayload> {
...
Please perform comprehensive analysis, identify the absolute best low-competition primary focus keyword, and return the fully completed JSON output. Ensure all HTML inside "long_content" is beautifully structured and complete.`;
  ...
  const result = await engine.prompt(userPrompt, systemPrompt, {
    temperature: 0.65,
    maxTokens: 2500,
    ...(specifiedModel ? { model: specifiedModel } : {}),
  });

  const cleanedContent = result.content.trim().replace(/^```json|```$/g, "").trim();

  try {
    const parsed: AiSeoPayload = JSON.parse(cleanedContent);
    return parsed;
  } catch (error) {
...
async function generateToolSeoWithRetry(
  tool: any,
  suggestedKeywords: string[],
  engine: AIEngine,
  retries = 3,
...
async function run() {
  ...
  try {
    ...
    for (const tool of tools) {
      ...
      if (!force) {
        const existingPage = await DB(T.TOOL_PAGES).where({ tool_slug: tool.slug }).first();
      ...
      let generatedData: AiSeoPayload;
      try {
      ...
      console.log(`   Features Count: ${generatedData.features?.length}`);
      console.log(`   FAQs Count: ${generatedData.faqs?.length}`);
      ...
      if (dryRun) {
        ...
        await DB.transaction(async (trx) => {
          ...
          const existingPage = await trx(T.TOOL_PAGES).where({ tool_slug: tool.slug }).first();
          ...
          if (existingPage) {
            await trx(T.TOOL_PAGES).where({ id: existingPage.id }).update(pageData);
          } else {
            await trx(T.TOOL_PAGES).insert({
              ...pageData,
